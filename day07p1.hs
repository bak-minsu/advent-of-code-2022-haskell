...
data FileSystemNode = Directory String Int [FileSystemNode] | File String Int deriving (Show)

getNodeSize :: FileSystemNode -> Int
getNodeSize (File name size) = size
getNodeSize (Directory name size children) = sum $ map getNodeSize children

updateNodeSize :: FileSystemNode -> FileSystemNode
updateNodeSize (File name size) = File name size
updateNodeSize (Directory name size children) = Directory name (getNodeSize (Directory name size children)) children
...
isSmallDirectory :: FileSystemNode -> Bool
isSmallDirectory (Directory _ size _) = size <= 100000
isSmallDirectory _                    = False

findSmallDirectories :: Trail -> [FileSystemNode]
findSmallDirectories (Trail path (Directory "/" size children)) = filterFileSystem isSmallDirectory (Directory "/" size children)
findSmallDirectories trail                                      = findSmallDirectories $ goRoot trail

sumNodeSizes :: [FileSystemNode] -> Int
sumNodeSizes []          = 0
sumNodeSizes (File _ size:rest)        = size + sumNodeSizes rest
sumNodeSizes (Directory _ size _:rest) = size + sumNodeSizes rest

debug :: [String] -> String
debug = show . sumNodeSizes . findSmallDirectories . calculateSizes . buildFileSystem root . convCommand

main :: IO ()
main = do args <- getArgs
          case args of
               [filename] -> do content  <- getFileLines filename
                                print . sumNodeSizes . findSmallDirectories . calculateSizes . buildFileSystem root . convCommand $ content
