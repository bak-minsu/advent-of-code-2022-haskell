...
maxXFormations formations = maximum $ map fst $ concat formations

maxYFormations :: Formations -> Int
maxYFormations formations = maximum $ map snd $ concat formations
...
spawnSand scan = SandState scan (500,0) (500, 1)

dropSand :: SandState -> Maybe SandState
dropSand (SandState scan prev next) | prev == next = Just $ SandState scan prev next
...
repeatUntilVoid :: ScanState -> ScanState
repeatUntilVoid (ScanState cave sandCount) = case dropSand $ spawnSand cave of
                                                  Nothing        -> ScanState cave sandCount
                                                  Just sandState -> repeatUntilVoid $ ScanState (drawPoint cave (prevSand sandState) SAND) (sandCount + 1)

main :: IO ()
main = do args <- getArgs
          case args of
               [filename] -> do content <- lines <$> readFile filename
                                print . sandCount . repeatUntilVoid . initScanState . initCaveScan . toFormations $ content
